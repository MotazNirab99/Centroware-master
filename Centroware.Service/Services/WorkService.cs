...
namespace Centroware.Service.Services
{
    public class WorkService : IWorkService
    {
        ...
        public async Task<Article> AddArticle(CreateArticleDto input)
        {
            ...
            {
                article.Image = await _fileService.SaveFile(input.ImageFile, "Images");
            }
        ...
        public async Task<bool> Delete(int id)
        {
            if (id > 0)
            {
                ...
                if (work != null)
                {
                    var articlas = await _articleRepository.Find(x => x.WorkId == id);
                    if (articlas != null && articlas.Any())
                        await _articleRepository.DeleteRangeAsync(articlas);
